...
class SentenceTransformerEnhance(nn.Sequential):
    """
    ...
    def start_multi_process_pool(self, target_devices: List[str] = None):
        ...
        for cuda_id in target_devices:
            p = ctx.Process(target=SentenceTransformerEnhance._encode_multi_process_worker,
    ...
    def smart_batching_collate(self, batch):
        ...
        label_dict["match_label"] = labels[:,0]

        level_single = labels[:,1]
        ...
        for idx in range(num_texts):
            ...
            # l = torch.where(input_ids == self._first_module().tokenizer.get_vocab()['[LEVEL]'], 0, l)
            l = torch.where(input_ids == self._first_module().tokenizer.get_vocab()['lepred'], 0, l)
            ...
            level_label = torch.full(labels_mlm.shape,-100)
            level_s = level_single.unsqueeze(1).repeat(1,labels_mlm.size(1))
            level = torch.where(input_ids == self._first_module().tokenizer.get_vocab()['lepred'],level_s,level_label)
            label_dict["level_label"] = level

        return sentence_features, label_dict

    def smart_batching_chatgptcollate(self, batch):
        """
        Transforms a batch from a SmartBatchingDataset to a batch of tensors for the model
        Here, batch is a list of tuples: [(tokens, label), ...]
    ...
    def smart_batching_collate_student(self, batch):
        """
        Transforms a batch from a SmartBatchingDataset to a batch of tensors for the model
        Here, batch is a list of tuples: [(tokens, label), ...]
        ...
        labels = torch.tensor(labels)
        embedd = torch.tensor([item.cpu().detach().numpy() for item in embeddings])


        label_dict = {}
        level_single = labels
        label_dict["teacher_embedding"] = embedd
    ...
    def smart_batching_collate_param_mask(self, batch):
        """
        Transforms a batch from a SmartBatchingDataset to a batch of tensors for the model
        Here, batch is a list of tuples: [(tokens, label), ...]
        ...
        for idx in range(num_texts):
            if idx == 0:
                ...
                # not parameter mask
                l = torch.where(token_type_ids == 1, 0, l)
                l = torch.where(input_ids == self._first_module().tokenizer.get_vocab()['[SEP]'], 0, l)
                l = torch.where(input_ids == self._first_module().tokenizer.get_vocab()['[CLS]'], 0, l)
                special_tokens_mask = l.bool()
                probability_matrix.masked_fill_(~special_tokens_mask, value=0.0)
                ...
                param_mlm = labels_mlm.clone()
                # parameter mask
                p = torch.ones(labels_mlm.shape, dtype=torch.long)
                p = torch.where(token_type_ids == 0, 0, p)
                p = torch.where(input_ids == self._first_module().tokenizer.get_vocab()['[SEP]'], 0, p)
                p = torch.where(input_ids == self._first_module().tokenizer.get_vocab()['[CLS]'], 0, p)
                probability_matrix2 = torch.full(input_ids.shape, 0.15)
                special_tokens_mask2 = p.bool()
                probability_matrix2.masked_fill_(~special_tokens_mask2, value=0.0)
                masked_indices2 = torch.bernoulli(probability_matrix2).bool()
                param_mlm[~masked_indices2] = -100
                input_ids[masked_indices2] = self._first_module().tokenizer.convert_tokens_to_ids("[MASK]")

                tokenized["input_ids"] = input_ids
                label_dict["mlm_label"] = labels_mlm
                label_dict["param_label"] = param_mlm
    ...
    def fit(self,
        ...
        for dataloader in dataloaders:
            ...
            # dataloader.collate_fn = self.smart_batching_collate_student
            # dataloader.collate_fn = self.smart_batching_collate_param_mask
            dataloader.collate_fn = self.smart_batching_chatgptcollate
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            if epoch % 10 == 0 and epoch != 0:
                ...
                # torch.save(loss_model.level.state_dict(),output_path + '_' + str(epoch) + '_levelpred.pth')

        if evaluator is None and output_path is not None:  # No evaluator, but output path: save final model version
            self.save(output_path)
            # torch.save(loss_model.level.state_dict(),output_path + '_levelpred.pth')
    ...
    @staticmethod
    def load(input_path):
        return SentenceTransformerEnhance(input_path)
